...
A directed graph consists of a set of nodes and a set of edges, where edges are a tuple of two node: a source and a target node of that edge. 
...
With this extended graph model, there is also a need to give rules for certain types of models, which is the role of \emph{domain modeling}.
A \emph{domain model} is a metamodel, defining how the graph model of a system is built up, i.e.\ what attributes a node has, how edges can be 
...
For example, if we want to express

\viatra{} Query Language syntax conform with this structure, and this way our expressions are capable of local search planning.
